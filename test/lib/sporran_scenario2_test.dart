...
void main() async {
  ...
  await deleter.deleteDatabase('sporranscenariotest2');
  ...
  group('9. Scenario Tests 2 - ', () {
    ...
    test('3. Create Attachment Online docid1 Attachment 1', () async {
      ...
      final res = await sporran9.putAttachment('9docid1', attachment);
      expect(res.ok, isTrue);
    ...
    test('4. Create Attachment Online docid1 Attachment 2', () async {
      final dynamic attachment = JsonObjectLite<dynamic>();
    ...
    test('5. Create Attachment Online docid2 Attachment 1', () async {
      final dynamic attachment = JsonObjectLite<dynamic>();
      attachment.attachmentName = 'AttachmentName1';
      attachment.rev = docid2rev;
      attachment.contentType = 'image/png';
      attachment.payload = attachmentPayload;
      final res = await sporran9.putAttachment('9docid2', attachment);
      expect(res.ok, isTrue);
      expect(res.operation, Sporran.putAttachmentc);
    ...
    test('6. Sync Pause', () async {
      await Future.delayed(Duration(seconds: 5));
    });

    test('7. Delete Document Offline docid3', () async {
      sporran9.online = false;
      final res = await sporran9.delete('9docid3', docid3rev);
      expect(res.ok, isTrue);
    ...
    test('9. Put Document Offline Updated docid2', () async {
      var res = await sporran9.get('9docid2', docid2rev);
      expect(res.ok, isTrue);
      ...
      res = await sporran9.put('9docid2', document2, docid2rev);
      expect(res.ok, isTrue);
